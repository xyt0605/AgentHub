...
**请求**:
```bash
curl -X POST "http://localhost:8080/api/agenthub/ingestion/pipelines" \
...
**请求**:
```bash
curl -X POST "http://localhost:8080/api/agenthub/ingestion/tasks/upload" \
  -F "pipelineId=1" \
...
**请求**:
```bash
curl "http://localhost:8080/api/agenthub/ingestion/tasks/123"
```
...
API_BASE="http://localhost:8080/api/agenthub"
